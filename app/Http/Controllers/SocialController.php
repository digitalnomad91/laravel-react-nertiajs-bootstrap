...
class SocialController extends Controller
{
    ...
    public function callback($provider)
    {
        try {
            ...
            $isUser = User::where('social_login_id', $user->id)
            ...
            if ($isUser) {
                ...
                $upload_path = null;
                if ($user->avatar) {
                    $upload_path = $this->saveImage($user->avatar);
                    $createUser->profile_photo_path = $upload_path;
                    $createUser->save();
                }
        ...
        if (is_array($wrapper_data)) {
            foreach (array_keys($wrapper_data) as $hh) {
                if (substr($wrapper_data[$hh], 0, 19) == 'Content-Type: image') {
                    ...
                    if (preg_match($pattern, substr($wrapper_data[$hh], 0), $match) !== false) {
